...
* **Package version** lives in `jira_telegram_bot/__init__.py` as `__version__` (e.g. `"2.40.89"`).
* **Every code change** must include a corresponding changelog entry at `docs/changelog/<version>.md` where `<version>` matches `__version__` in `jira_telegram_bot/__init__.py`.
...
* When bumping the version, update `__version__` in `jira_telegram_bot/__init__.py` and create a new changelog file for the new version.
