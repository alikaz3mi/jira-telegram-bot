...
* If the user chooses tasks → list current-sprint issues for selection.
* If the user skips → agent must infer mapping based on NLP and heuristics (harder path).
* All interactions are conversational; speech audio is converted to text via the existing Speech-to-Text service.
...
1. **Prompt Template**  
   ...
   * Inputs: `raw_transcript`, `selected_issue_keys`, `assignee`, `sprint_label`
   * Output schema: `{"issue_key": str, "progress": str, "blockers": str}`
2. **AI-Agent Service**  
...
/write-unit-tests: paths=jira_telegram_bot
/write-integrated-tests: paths=frameworks/scheduler,frameworks/telegram
/generate docs
