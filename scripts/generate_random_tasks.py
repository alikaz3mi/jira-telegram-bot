...
class RandomTaskGenerator:
    ...
    def generate_task(
        ...
        is_subtask = (
            parent_key
            and parent_key.startswith(project_key)
            and project_key == "SILMARIN"
        )
    ...
    async def generate_random_tasks(self, total_count: int = 100) -> None:
        ...
        try:
            ...
            for i in range(story_count):
                project_key = random.choice(RandomTaskConstants.PROJECT_KEYS)
                epic_key = (
                    random.choice(self.created_epics)
                    if self.created_epics and random.random() > 0.3
                    else None
                )
                ...
                try:
                    ...
                    created_count += 1
                    LOGGER.info(
                        f"Created story {story_issue.key}: {story_data.summary}",
                    )
                except Exception as e:
            ...
            for i in range(task_count):
                project_key = random.choice(RandomTaskConstants.PROJECT_KEYS)
                parent_key = (
                    random.choice(self.created_stories)
                    if self.created_stories and random.random() > 0.6
                    else None
                )
        ...
        LOGGER.info(
            f"Task generation completed. Created {created_count} out of {total_count} requested tasks",
        )
        LOGGER.info(
            f"Created {len(self.created_epics)} epics and {len(self.created_stories)} stories",
        )
...
async def main():
    ...
    try:
        LOGGER.info("🚀 Starting random task generation...")
        ...
        LOGGER.info(
            f"✅ Successfully initialized task repository: {type(task_repository).__name__}",
        )
        ...
        LOGGER.info("✅ Random task generation completed successfully!")
