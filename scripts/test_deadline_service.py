...
from jira_telegram_bot.use_cases.team_evaluation.services.deadline_service import DeadlineService
...
def test_deadline_service():
    ...
    delivery_time_aware = base_time.replace(tzinfo=timezone.utc)  # Delivered today (aware)
    
    print("Testing timezone handling...")
    print(f"Due date (naive): {due_date_naive} - tzinfo: {due_date_naive.tzinfo}")
    print(f"Delivery time (aware): {delivery_time_aware} - tzinfo: {delivery_time_aware.tzinfo}")
