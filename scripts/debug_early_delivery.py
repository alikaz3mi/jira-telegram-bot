...
def debug_early_delivery():
    ...
    print("=== دیباگ محاسبه بونوس تعجیل ===\n")
    ...
    print(f"Deadline delta hours: {avg_deadline_delta_hours}")
    print(f"Deadline delta days: {deadline_delta_days}")
    ...
    print(f"Early bonus: {early_bonus}")
    print(f"S_deadline: {s_deadline}")
    ...
    print(f"S_worklog: {s_worklog}")
    print(f"S_high: {s_high}")
    print(f"S_defects: {s_defects}")
    ...
    print(f"\nFinal calculation:")
    print(f"0.25 × {s_deadline} + 0.20 × {s_worklog} + 0.40 × {s_high} + 0.15 × {s_defects}")
    print(f"= {0.25 * s_deadline} + {0.20 * s_worklog} + {0.40 * s_high} + {0.15 * s_defects}")
    print(f"= {composite_score}")
    print(f"Rounded: {round(composite_score)}")
