#!/usr/bin/env python3
"""Script to sync Jira stories to Google Sheets.
...
async def setup_components():
    ...
    Returns:
        SyncStoryToSheetsUseCase instance.
    """
    try:
        container = get_container()
        sync_use_case = container[SyncStoryToSheetsUseCase]
        return sync_use_case
    except Exception as e:
        LOGGER.error(f"Error setting up story synchronization components: {e}")
        raise


async def run_sync_once(
    use_case: SyncStoryToSheetsUseCase,
...
async def test_connection(use_case: SyncStoryToSheetsUseCase):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Sync Jira stories to Google Sheets",
    )
    ...
    parser.add_argument(
        "--days-back",
        type=int,
        help="Days to look back for changes (sync command only). Overrides --full.",
    )
