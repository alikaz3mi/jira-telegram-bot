...
class FullReportGenerator:
    """Generate comprehensive reports and dashboards based on task data."""
    
    def __init__(self, jira_repository: TaskManagerRepositoryInterface):
        """Initialize the full report generator.
    ...
    def remove_illegal_chars(value: Any) -> Any:
        ...
        Specifically removes ASCII control characters 0-8, 11-12, 14-31, 127-159.
        
        Args:
            value: The string value to clean up
            
        Returns:
            Cleaned string with illegal characters removed
        """
    ...
    def sanitize_dataframe(df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df.columns:
            if df[col].dtype == object:
                df[col] = df[col].apply(FullReportGenerator.remove_illegal_chars)
        return df

    def get_tasks_info(self, project_key: str = "PARSCHAT") -> List[Dict[str, Any]]:
        """Fetch tasks from Jira and return as a list of dictionaries.
        
        Includes Sprint data, Epic references, etc.
        
        Args:
            project_key: Jira project key to fetch tasks for
    ...
    @staticmethod
    def create_sprint_progress_dashboard(df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
        ...
        Returns:
            Dictionary of DataFrames to be written to Excel sheets
        """
    ...
    @staticmethod
    def create_sprint_progress_dashboard_by_sprint(df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
        """Create data frames for each sprint's progress metrics.
    ...
    @staticmethod
    def create_team_productivity_workload_dashboard(df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
        """Create data frames for team productivity and workload metrics.
        ...
        Returns:
            Dictionary of DataFrames with productivity metrics
        """
    ...
    def generate_full_report(self, project_key: str = "PARSCHAT") -> str:
        """Generate a full Excel report with multiple sheets and dashboards.
        
        Args:
            project_key: Jira project key to generate report for
            
        Returns:
            Filename of the generated report
        """
        # 1) Fetch tasks from Jira
        tasks = self.get_tasks_info(project_key)
        ...
        sprint_progress_data = self.create_sprint_progress_dashboard(df)
        sprint_progress_per_sprint = self.create_sprint_progress_dashboard_by_sprint(df)
        team_productivity_data = self.create_team_productivity_workload_dashboard(df)
        ...
        for key, sdf in sprint_progress_data.items():
            sprint_progress_data[key] = self.sanitize_dataframe(sdf)
        for key, sdf in sprint_progress_per_sprint.items():
            sprint_progress_per_sprint[key] = self.sanitize_dataframe(sdf)
        for key, sdf in team_productivity_data.items():
            team_productivity_data[key] = self.sanitize_dataframe(sdf)
...
if __name__ == "__main__":
    ...
    report_generator = FullReportGenerator(jira_repository)
    report_generator.generate_full_report("PARSCHAT")
