...
class SendDeadlineAlertsUseCase:
    ...
    async def _convert_issues_to_alerts(self, issues: List[Issue]) -> List[DeadlineAlert]:
    ...
    async def _create_deadline_alert(self, issue: Issue, now: datetime) -> Optional[DeadlineAlert]:
        """Create a deadline alert from a Jira issue."""
        due_date = self._parse_date_field(getattr(issue.fields, "duedate", None))
        target_end = self._parse_date_field(getattr(issue.fields, "customfield_10110", None))
    ...
    async def _send_personal_notifications(self, alerts: List[DeadlineAlert]) -> Dict[str, int]:
        ...
        for jira_username, assignee_alerts in alerts_by_assignee.items():
            try:
                ...
                LOGGER.error(f"Error sending personal notification to {jira_username}: {e}")
                continue
    ...
    async def _send_group_notifications(self, alerts: List[DeadlineAlert]) -> Dict[str, int]:
        ...
        for chat_id in group_chat_ids:
            try:
                ...
                urgent_alerts = [alert for alert in alerts if alert.urgency_level in ["overdue", "today", "urgent"]]
