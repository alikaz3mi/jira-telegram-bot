...
class ApiCalendarRepository(CalendarRepositoryInterface):
    ...
    async def _get_holidays_for_jalali_year(self, jalali_year: int) -> Set[date]:
        ...
        results = await self._execute_with_rate_limit([
            self._check_holiday_for_date(jalali_date) 
            for jalali_date in sample_dates
        ])
    ...
    def _get_sample_dates_for_jalali_year(self, jalali_year: int) -> list[jdatetime.date]:
        ...
        for month in range(1, 13):  # 12 months
            ...
            try:
                ...
                LOGGER.debug(f"Error processing month {month} in year {jalali_year}: {e}")
                continue
        
        LOGGER.debug(f"Generated {len(sample_dates)} sample dates for Jalali year {jalali_year}")
        return sample_dates
    ...
    async def _execute_with_rate_limit(self, tasks, batch_size: int = 5, delay: float = 0.2):
